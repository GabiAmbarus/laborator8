				...
				try {
					...
					while(!Thread.currentThread().isInterrupted()){
						...
						if(message != null) {
							...
							printWriter.flush();
							Message objMessage = new Message(message,Constants.MESSAGE_TYPE_SENT);
							conversationHistory.add(objMessage);
							if (context != null) {
								  ...
								  if (fragment instanceof ChatConversationFragment && fragment.isVisible()) {
								    ...
								    chatConversationFragment.appendMessage(objMessage);
								  }
							}
						}
					}
			...
			if (bufferedReader != null) {
				try {
					...
					while(!Thread.currentThread().isInterrupted()){
						...
						if(message != null) {
							Message objMessage = new Message(message,Constants.MESSAGE_TYPE_RECEIVED);
							conversationHistory.add(objMessage);
							if (context != null) {
								  ChatActivity chatActivity = (ChatActivity)context;
								  FragmentManager fragmentManager = chatActivity.getFragmentManager();
